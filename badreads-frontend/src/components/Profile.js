...
export default class Home extends Component {
    ...
    componentDidMount() {
        axios.get(`http://${process.env.REACT_APP_PRODUCTION_IP}/userbook/${getUser().userId}`)
            .then(res => {
    ...
    all = () => {
        ...
        axios.get(`http://${process.env.REACT_APP_PRODUCTION_IP}/userbook/${userId}`)
            .then(res => {
                const userBooks = res.data.map((val) => {
                    return val.bookId
                })
    ...
    readAction = (e, value) => {
        ...
        let userId = getUser().userId
        axios.get(`http://${process.env.REACT_APP_PRODUCTION_IP}/userbook/${value}/${userId}`)
            .then(res => {
                const userBooks = res.data.map((val) => {
                    return val.bookId
                })
